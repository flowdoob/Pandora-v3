...
bool Hooks::WriteUsercmdDeltaToBuffer( int m_nSlot, void* m_pBuffer, int m_nFrom, int m_nTo, bool m_bNewCmd ) {
	if( g_tickbase.m_shift_data.m_ticks_to_shift <= 0 )
		return g_hooks.m_client.GetOldMethod<WriteUsercmdDeltaToBuffer_t>( 23 )( this, m_nSlot, m_pBuffer, m_nFrom, m_nTo, m_bNewCmd );
	...
	//
	//auto m_pnBackupCmds = reinterpret_cast < int* > ( frame_ptr + 0xFD8 );
	//auto m_pnNewCmds = reinterpret_cast < int* > ( frame_ptr + 0xFDC );
	...
	*m_pnNewCmds = m_nTotalNewCmds;
	...
	for( m_nTo = m_nNextCmd - m_nNewCmds + 1; m_nTo <= m_nNextCmd; m_nTo++ ) {
		if( !g_hooks.m_client.GetOldMethod<WriteUsercmdDeltaToBuffer_t>( 23 )( this, m_nSlot, m_pBuffer, m_nFrom, m_nTo, true ) )
			return false;
