...
void Animations::UpdatePlayer( Player* pEntity ) {
	//static bool& invalidate_bone_cache = **reinterpret_cast< bool** >( pattern::find( g_csgo.m_client_dll, XOR( "80 3D ? ? ? ? ? 74 16 A1 ? ? ? ? 48 C7 81" ) ).as<uint32_t>( ) ); // add 2
...
void Animations::UpdateLocalAnimations( ) {
	...
	state->m_update_delta = math::Max( 0.0f, g_csgo.m_globals->m_curtime - state->m_time ); // negative values possible when clocks on client and server go out of sync..
	...
	if( g_cl.m_animate ) {
		...
		if( !g_csgo.m_cl->m_choked_commands ) {
			g_cl.m_rotation.y = state->m_goal_feet_yaw;
...
void Animations::UpdateFakeAnimations( ) {
	...
	if( allocate || change ) {
		...
		g_cl.m_fake_state->m_update_delta = fmaxf( g_csgo.m_globals->m_curtime - g_cl.m_fake_state->m_time, 0.0 );
